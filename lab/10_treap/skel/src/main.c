...
void __ascending_nodes(Node *node, int *keys, int *num_keys)
{
	if (!node)
		return;
		
	__ascending_nodes(node->left, keys, num_keys);
	keys[(*num_keys)++] = *(int *)node->data;
	__ascending_nodes(node->right, keys, num_keys);
}
...
{
	__ascending_nodes(node, keys, num_keys);
}
